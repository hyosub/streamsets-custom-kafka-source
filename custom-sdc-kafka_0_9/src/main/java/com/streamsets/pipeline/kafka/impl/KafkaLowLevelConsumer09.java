...
public class KafkaLowLevelConsumer09 implements SdcKafkaLowLevelConsumer {

  private static final Logger LOG = LoggerFactory.getLogger(KafkaLowLevelConsumer09.class);
  ...
  public KafkaLowLevelConsumer09(
          String topic,
          int partition,
          HostAndPort broker,
          int minFetchSize,
  ...
  public List<FullMessageAndOffset> read(long offset) throws StageException {
    ...
    List<FullMessageAndOffset> partitionToPayloadMapArrayList = new ArrayList<>();
    for (kafka.message.MessageAndOffset messageAndOffset : fetchResponse.messageSet(topic, partition)) {
      ...
      payload.get(bytes);
      FullMessageAndOffset partitionToPayloadMap = new FullMessageAndOffset(
              key,
              bytes,
    ...
    for(HostAndPort broker : brokers) {
      ...
      try {
        LOG.info(
                ...
                broker.getHostText(),
                broker.getPort(),
                METADATA_READER_TIME_OUT, BUFFER_SIZE,
